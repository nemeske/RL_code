...
% init
c = 2; %  the index of the current phrase
tau_c = 2; % start time of the cth phrase
...
for t = 2:10000
    ...
    if N(X(t),A(t)) > 0 % context seen
        ...
        A(t+1) = randi(3);
        for s =tau_c:t
            N(X(s),A(s)) = N(X(s),A(s)) + 1; % updating context
            P_h((s-1),A(s)) = (N(X(s-1),A(s)) +0.5 )/(sum(N(:,A(s))) + 3/2);
            
            tempP = P_h(X(s),A(s))*g+alpha*J_h(X(s),A(s));
            %tempP = sum(tempP(:,A(s)));
            J_h(X(s-1),A(s))= tempP;
        end
        c = c + 1;
